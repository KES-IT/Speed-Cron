...
func (u *uAuth) DeviceAuth(initData *g_structs.InitData) (err error) {
	...
	// 进行设备认证
	glog.Debug(context.Background(), "开始进行设备认证", g_consts.BackendBaseUrl()+g_consts.AuthBackendUrl)
	response, err := g.Client().SetTimeout(5*time.Second).Post(context.TODO(), g_consts.BackendBaseUrl()+g_consts.AuthBackendUrl, g.Map{
